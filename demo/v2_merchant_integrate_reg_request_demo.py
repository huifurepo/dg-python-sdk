...
def build_extend_infos():
    ...
    extend_infos = dict()
    # 经营简称
    extend_infos["short_name"] = "姜雨"
    # 小票名称
    extend_infos["receipt_name"] = "汇付天下"
    # 商户英文名称
    extend_infos["mer_en_name"] = "huifu"
    # 所属行业
    extend_infos["mcc"] = "7273"
    # 营业执照类型
    extend_infos["license_type"] = "NATIONAL_LEGAL_MERGE"
    # 营业执照编号
    extend_infos["license_code"] = "914403001921834459"
    # 营业执照有效期类型
    extend_infos["license_validity_type"] = "0"
    # 营业执照有效期开始日期
    extend_infos["license_begin_date"] = "19831108"
    # 营业执照有效期截止日期
    extend_infos["license_end_date"] = "20380831"
    # 注册详细地址
    extend_infos["reg_detail"] = "深圳市宝安区新安街道海旺社区N26区海秀路2021号荣超滨海大厦A座2111"
    # 注册省
    ...
    # 客服电话
    extend_infos["service_phone"] = "15556622000"
    # 商户主页URL
    extend_infos["mer_url"] = "http://www.baidu.com"
    # 商户ICP备案编号
    extend_infos["mer_icp"] = "苏ICP备15042526号"
    # 开户许可证核准号
    extend_infos["open_licence_no"] = "123456789"
    # 法人信息
    extend_infos["legal_info"] = getLegalInfo()
    # 签约人
    # extend_infos["sign_user_info"] = getSignUserInfo()
    # 管理员账号
    extend_infos["login_name"] = "LG02022072707540497330158089012"
    # 是否通知商户标识
    extend_infos["sms_send_flag"] = "1"
    # 协议信息实体
    extend_infos["agreement_info"] = getAgreementInfo()
    # 业务开关配置
    extend_infos["biz_conf"] = getBizConf()
    # 微信配置对象
    extend_infos["wx_conf_list"] = getWxConfList()
    # 实名认证信息
    extend_infos["wx_realname_info"] = getWxRealnameInfo()
    # 支付宝配置对象
    extend_infos["ali_conf_list"] = getAliConfList()
    # 银联二维码配置
    extend_infos["union_conf_list"] = getUnionConfList()
    # 银联小微入驻信息实体
    extend_infos["union_micro_info"] = getUnionMicroInfo()
    # 余额支付配置实体
    extend_infos["balance_pay_config"] = getBalancePayConfig()
    # 银行卡业务配置实体
    extend_infos["bank_card_conf"] = getBankCardConf()
    # 花呗分期费率配置实体
    extend_infos["hb_fq_fee_config"] = getHbFqFeeConfig()
    # 补贴支付
    ...
    # 异步消息接收地址
    extend_infos["async_return_url"] = "rmq://C_SSPM_YMFZ_AUDIT"
    # 业务开通结果异步消息接收地址
    extend_infos["busi_async_return_url"] = "virgo://http://192.168.85.157:30031/sspm/testVirgo"
    # 交易异步应答地址
    extend_infos["recon_resp_addr"] = "archer://C_SSPM_NSPOSM_BUSIRESULT"
    return extend_infos


class TestV2MerchantIntegrateRegRequestDemo(unittest.TestCase):
    ...
    def test_request(self):
        ...
        # 接口请求对象
        request = dg_sdk.V2MerchantIntegrateRegRequest()
        request.req_seq_id = ""
        request.req_date = ""
        request.upper_huifu_id = "6666000105215341"
        request.ent_type = "3"
        request.reg_name = "天马微电子股份有限公司"
        request.busi_type = "1"
        request.detail_addr = "深圳市宝安区新安街道海旺社区N26区海秀路2021号荣超滨海大厦A座2111"
        request.prov_id = "310000"
        request.area_id = "310100"
        request.district_id = "310104"
        request.contact_info = getContactInfo()
        request.card_info = getCardInfo()
        request.cash_config = getCashConfig()
        request.settle_config = getSettleConfig()
