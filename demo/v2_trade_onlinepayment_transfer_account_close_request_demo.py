import unittest
import dg_sdk
import json
from demo.demo_config import *



def build_extend_infos():
    """
    非必填字段

    :return: 非必填字段组成的字典
    """
    extend_infos = dict()
    # 银行卡号
    extend_infos["bank_card_no"] = ""
    # 入账标识
    extend_infos["in_acct_flag"] = "YDNUCSV2JV7BAXYN"
    return extend_infos


class TestV2TradeOnlinepaymentTransferAccountCloseRequestDemo(unittest.TestCase):

    def setUp(self):
        dg_sdk.DGClient.mer_config = dg_sdk.MerConfig(PRIVATE_KEY, PUBLIC_KEY, SYS_ID, PRODUCT_ID)

        print("setup")


    # 银行大额支付关单 - 示例
    def test_request(self):

        # 接口请求对象
        request = dg_sdk.V2TradeOnlinepaymentTransferAccountCloseRequest()
        request.req_seq_id = ""
        request.req_date = ""
        request.huifu_id = "6666000003100616"
        request.org_req_seq_id = "1399999399862667"
        request.org_req_date = "20221020"

        # 所有非必填字段字典
        extend_infos = build_extend_infos()

        result = request.post(extend_infos)

        print(result)
        assert result["resp_code"] != ""